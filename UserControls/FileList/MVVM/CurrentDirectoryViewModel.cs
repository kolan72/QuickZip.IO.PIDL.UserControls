...
{
    public class CurrentDirectoryViewModel : DirectoryViewModel, IEnumerable<FileListViewItemViewModel>
    {
        #region Constructor
            ...
            : base(rootModel, model)
        {
            ...
            _subEntries.SortDescriptions.Add(new SortDescription("FullName", ListSortDirection.Ascending)); //! AK изменение на Name не влияет на сбой сортировки!

            if (!model.EmbeddedDirectoryEntry.Exists)
            {
                return;
            }
        ...
        public void SetInternalCollection(ObservableCollection<FileListViewItemViewModel> subEntriesInternal, ExComparer.SortCriteria sortCriteria, ListSortDirection sortDirection)
        {
            _subEntriesInternal = subEntriesInternal;
            _subEntries.Source = SubEntriesInternal;

            ////var c = _subEntries.SortDescriptions.Count;

            _subEntries.SortDescriptions.Clear();
            //TODO Добавить сортировку по типу
            if (sortDirection == ListSortDirection.Descending)
            {
                _subEntries.SortDescriptions.Add(new SortDescription("IsDirectory", ListSortDirection.Ascending));
                switch (sortCriteria)
                {
                    case System.IO.Tools.ExComparer.SortCriteria.sortByCreationTime:
                        _subEntries.SortDescriptions.Add(new SortDescription("CreationTime", ListSortDirection.Descending));
                        break;
                    case System.IO.Tools.ExComparer.SortCriteria.sortByFullName:
                        _subEntries.SortDescriptions.Add(new SortDescription("FullName", ListSortDirection.Descending));
                        break;
                    case System.IO.Tools.ExComparer.SortCriteria.sortByName:
                        _subEntries.SortDescriptions.Add(new SortDescription("Name", ListSortDirection.Descending));
                        break;
...
>>>>>>> c013b83305f0a8b2d4fa3e9f43e2a72c5cdbd294
            ...
            else
            {
                ...
                {
                    case System.IO.Tools.ExComparer.SortCriteria.sortByCreationTime:
                        _subEntries.SortDescriptions.Add(new SortDescription("CreationTime", ListSortDirection.Ascending));
                        break;
                    case System.IO.Tools.ExComparer.SortCriteria.sortByFullName:
                        _subEntries.SortDescriptions.Add(new SortDescription("FullName", ListSortDirection.Ascending));
                        break;
                    case System.IO.Tools.ExComparer.SortCriteria.sortByName:
                        _subEntries.SortDescriptions.Add(new SortDescription("Name", ListSortDirection.Ascending));
                        break;
                    case ExComparer.SortCriteria.sortByLastWriteTime:
...
>>>>>>> c013b83305f0a8b2d4fa3e9f43e2a72c5cdbd294
            ...
            //_subEntries.SortDescriptions.Add(new SortDescription("IsDirectory", ListSortDirection.Descending));
            //          Убрано, похоже сбивает сортировку
            ////_subEntries.SortDescriptions.Add(new SortDescription("FullName", ListSortDirection.Ascending));
        }

        public virtual object FetchDataAsync()
        {
            return null;
        }
        ...
        /// <summary>
        /// Показывает, может ли выборка быть ограниченной сверху по числу записей
        /// </summary>
        ...
        {
            if (!((Model.DirectoryModel)EmbeddedModel).EmbeddedDirectoryEntry.Exists) //+Ak
            {
                return;
            }
        ...
        {
            lock (_lockObj)//_selectedSubEntries <>AK
                if (isSelected)
                {
                    if (!_dictSelectedSubEntries.ContainsKey(model.EmbeddedModel)) //!_selectedSubEntries.Contains(model)
                    {
                        _selectedSubEntries.Add(model);
                        _dictSelectedSubEntries.Add(model.EmbeddedModel, model.EmbeddedModel);
                    }
                }
                else
                {
                    if (_dictSelectedSubEntries.ContainsKey(model.EmbeddedModel)) //_selectedSubEntries.Contains(model)
                    {
                        _dictSelectedSubEntries.Remove(model.EmbeddedModel);
        ...
        public void UnselectAll(FileListViewItemViewModel itemOfUnselected)
        {
            foreach (FileListViewItemViewModel item in SubEntries.View)
                item.IsSelected = false;

            itemOfUnselected.IsSelected = true;
        }



        //+AK
        public void UnselectAllOnlySelected(List<FileListViewItemViewModel> lst)
        {
            foreach (FileListViewItemViewModel item in lst)
            {
                item.IsSelected = false;
            }
        }
        ...
        private Dictionary<ExModel, ExModel> _dictSelectedSubEntries = new Dictionary<ExModel, ExModel>();
        ...
        public bool IsViewModelJustRefreshed { get;  set; }
        ...
        #region Implementation IEnumerable<FileListViewItemViewModel>

        public IEnumerator<FileListViewItemViewModel> GetEnumerator()
        {
            foreach (FileListViewItemViewModel item in SubEntries.View)
            {
                yield return item;
